...
export function ToolStepCard({ step }: ToolStepCardProps) {
  ...
  return (
    <div className="rounded-xl bg-bg-200 border border-ink-900/5 overflow-hidden">
      <Collapsible.Root open={isExpanded} onOpenChange={setIsExpanded}>
        ...
        >
          ...
          <div className="flex-1 flex items-center gap-2 min-w-0 text-left">
            ...
            {toolInfo && (
              <span className="text-xs text-muted truncate">{toolInfo}</span>
            )}
          </div>
          {hasResult && (
            <ChevronDown
              className={`w-3.5 h-3.5 text-ink-400 transition-transform duration-200 shrink-0 ${
        ...
        {hasResult && (
          <Collapsible.Panel className="overflow-hidden">
            <div className="px-3 pb-2 pt-1 border-t border-ink-900/5">
              <pre
                className={`text-xs whitespace-pre-wrap break-words font-mono max-h-40 overflow-y-auto ${
                  step.isError ? "text-error" : "text-ink-600"
                }`}
              >
                {step.result}
              </pre>
